<template>
  <div>
    ...
    <div v-else class="p-6 space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow p-6 border border-gray-100 dark:border-slate-700">
          ...
          <div class="mt-4">
            <div class="text-sm text-gray-600 dark:text-gray-400">
              <span class="text-green-600 dark:text-green-400 font-medium">{{ stats.published_posts }}</span> publicados,
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow p-6 border border-gray-100 dark:border-slate-700">
          ...
          <div class="mt-4">
            <div class="text-sm text-gray-600 dark:text-gray-400">
              ...
              <span class="text-green-600 dark:text-green-400 font-medium">{{ stats.replied_contacts }}</span> respondidos
            </div>
            <div class="text-xs text-gray-500 mt-1">
          ...
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dl>
                ...
                <dd class="text-lg font-medium text-gray-900 dark:text-white">{{ stats.total_views?.toLocaleString() }}</dd>
              </dl>
            </div>
          </div>
          <div class="mt-4">
      ...
      <div v-if="monthlyStats.length" class="bg-white dark:bg-slate-800 rounded-lg shadow p-6 border border-gray-100 dark:border-slate-700">
        ...
        <div class="h-64 flex items-end justify-between space-x-1">
          <div v-for="month in monthlyStats" :key="month.month" class="flex-1 flex flex-col items-center">
            <div class="w-full flex flex-col items-center space-y-1">
              ...
              <div 
                class="w-6 bg-green-500 rounded-b"
                :style="{ height: `${Math.max(((month.views || 0) / 10 / maxMonthlyValue) * 200, 4)}px` }"
                :title="`${month.views} vistas`"
              ></div>
            </div>
            <span class="text-xs text-gray-500 dark:text-gray-400 mt-2 transform -rotate-45 origin-top-left">{{ month.month_short }}</span>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow border border-gray-100 dark:border-slate-700">
          ...
          <div class="divide-y divide-gray-200 dark:divide-slate-700 max-h-96 overflow-y-auto">
            ...
            <div v-for="post in recentPosts" :key="post.id" class="px-6 py-3 hover:bg-gray-50 dark:hover:bg-slate-700/50">
              <div class="flex items-start justify-between">
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-xs text-gray-400 mt-1">{{ post.time_ago || formatDate(post.created_at) }} • {{ post.views }} vistas</p>
                </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow border border-gray-100 dark:border-slate-700">
          ...
          <div class="divide-y divide-gray-200 dark:divide-slate-700 max-h-96 overflow-y-auto">
            ...
            <div v-for="contact in recentContacts" :key="contact.id" class="px-6 py-3 hover:bg-gray-50 dark:hover:bg-slate-700/50">
              <div class="flex items-start justify-between">
                <div class="flex-1 min-w-0">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow border border-gray-100 dark:border-slate-700">
          ...
          <div class="divide-y divide-gray-200 dark:divide-slate-700">
            ...
            <div v-for="post in popularPosts" :key="post.id" class="px-6 py-3 hover:bg-gray-50 dark:hover:bg-slate-700/50">
              <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                  <p class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ post.title }}</p>
                  <div class="flex items-center mt-1 space-x-2">
                    ...
                    <span class="text-xs text-gray-500">{{ post.published_at || formatDate(post.created_at) }}</span>
                  </div>
                </div>
                <div class="ml-4 flex-shrink-0 text-right">
                  <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ post.views?.toLocaleString() || 0 }}</p>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow border border-gray-100 dark:border-slate-700">
          ...
          <div class="divide-y divide-gray-200 dark:divide-slate-700">
            ...
            <div v-for="category in topCategories" :key="category.id" class="px-6 py-3 hover:bg-gray-50 dark:hover:bg-slate-700/50">
              <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                  <div class="flex items-center">
                    <div class="w-3 h-3 rounded-full mr-2" :style="{ backgroundColor: category.color || '#6b7280' }"></div>
                    <p class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ category.name }}</p>
                  </div>
                  <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    {{ category.published_posts_count || 0 }} publicados • {{ category.total_views?.toLocaleString() || 0 }} vistas
                  </p>
                </div>
                <div class="ml-4 flex-shrink-0 text-right">
                  <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ category.posts_count || 0 }}</p>
      ...
      <div v-if="performanceStats && (performanceStats.most_viewed_post || performanceStats.most_active_category)" class="bg-white dark:bg-slate-800 rounded-lg shadow p-6 border border-gray-100 dark:border-slate-700">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div v-if="performanceStats.most_viewed_post" class="text-center">
            ...
            <p class="text-lg font-medium text-gray-900 dark:text-white truncate">{{ performanceStats.most_viewed_post.title }}</p>
            <p class="text-sm text-blue-600 dark:text-blue-400">{{ performanceStats.most_viewed_post.views?.toLocaleString() || 0 }} vistas</p>
          </div>
          <div v-if="performanceStats.most_active_category" class="text-center">
            ...
            <p class="text-lg font-medium text-gray-900 dark:text-white">{{ performanceStats.most_active_category.name }}</p>
            <p class="text-sm text-green-600 dark:text-green-400">{{ performanceStats.most_active_category.published_posts_count }} posts</p>
          </div>
          <div class="text-center">
            <p class="text-sm text-gray-500 dark:text-gray-400">Promedio Contactos/Día</p>
            <p class="text-lg font-medium text-gray-900 dark:text-white">{{ performanceStats.avg_contacts_per_day || 0 }}</p>
